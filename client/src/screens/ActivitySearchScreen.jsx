    ...
    <div className="space-y-8 pb-20 animate-fade-in">
      ...
      <div>
        ...
        <p className="text-xs sm:text-sm text-slate-400 mt-1">
          Browse hundreds of authentic experiences, museum visits, culinary walking tours, and sunset adventures across the world.
        </p>
      </div>
      ...
      <div className="p-5 rounded-3xl glass-panel border border-slate-800 space-y-4">
        ...
        <form onSubmit={handleSearchSubmit} className="relative">
          ...
          <input
            ...
            className="w-full bg-slate-900/90 border border-slate-700/80 text-white rounded-2xl pl-11 pr-4 py-3 text-xs focus:outline-none focus:border-indigo-500 font-medium"
          />
        </form>
        ...
        {/* Secondary Filter: City selector & Max Price Slider */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-3 border-t border-slate-800">
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-slate-400 uppercase">Filter City:</span>
            <select
              value={selectedCityId}
              onChange={(e) => setSelectedCityId(e.target.value)}
              className="bg-slate-900/90 border border-slate-700 text-white rounded-xl px-3 py-1.5 text-xs focus:outline-none focus:border-indigo-500"
            >
          ...
          <div className="flex items-center gap-3">
            <span className="text-xs font-bold text-slate-400">Max Cost: ${maxPrice}</span>
            <input
              type="range"
              min="10"
              max="250"
      ...
      {loading ? (
        ...
        <div className="p-12 rounded-3xl glass-card text-center space-y-3">
          ...
          <h3 className="text-base font-bold text-white">No Activities Match Filter</h3>
          <p className="text-xs text-slate-400">Try loosening your budget limit or category filter.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {activities.map((act) => (
            ...
            >
              <div className="relative h-48 w-full overflow-hidden bg-slate-800">
                ...
                <div className="absolute top-3 left-3">
                  <span className="px-2.5 py-1 text-[10px] font-extrabold uppercase rounded-lg bg-slate-900/80 backdrop-blur-md text-indigo-300 border border-indigo-500/30">
                    {act.category}
                  </span>
                </div>

                <div className="absolute top-3 right-3">
                  <span className="px-2.5 py-1 text-[11px] font-extrabold rounded-lg bg-slate-900/80 backdrop-blur-md text-amber-300 border border-amber-500/30 flex items-center gap-1">
                    <Star className="w-3 h-3 fill-amber-400 text-amber-400" />
                    {act.rating}
                  </span>
                </div>

                <div className="absolute bottom-3 left-3 right-3 text-xs text-slate-200 font-semibold drop-shadow-md">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-800 text-xs">
                  <div>
                    ...
                    <span className="text-sm font-bold text-emerald-400">
                      ${act.cost} <span className="text-[10px] text-slate-400 font-normal">/ {act.duration_mins}m</span>
                    </span>
                  </div>

                  <button
                    onClick={() => handleOpenAddModal(act)}
                    className="flex items-center gap-1.5 px-3.5 py-2 text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-500 rounded-xl shadow-md shadow-indigo-600/30 transition hover:scale-102"
                  >
      ...
      {targetActivity && (
        <Modal
          ...
          title={`Add "${targetActivity.name}" to Itinerary`}
          maxWidth="max-w-md"
        >
          {userTrips.length === 0 ? (
            <div className="text-center py-6 space-y-3">
              <p className="text-xs text-slate-400">No active trips found in your account.</p>
            </div>
          ) : (
